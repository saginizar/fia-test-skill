...
```bash
curl.exe -sS -X POST "<api_base>/skill/submit" \
  -H "Content-Type: application/json" \
  --data @<temp-dir>/fia-payload.json
```
...
"Thanks — that's been sent to the team. We really appreciate it." Do not show
the raw response or the feedback_id unless asked.
